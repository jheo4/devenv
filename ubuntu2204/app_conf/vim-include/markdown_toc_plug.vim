...
let s:supportMarkdownStyles = ['GFM', 'Redcarpet', 'GitLab', 'Marked', 'Wiki']

let s:GFM_STYLE_INDEX = 0
let s:WIKI_STYLE_INDEX = 4
...
function! s:GetHeadingLink(headingName, markdownStyle)
    ...
    elseif a:markdownStyle ==# s:supportMarkdownStyles[s:WIKI_STYLE_INDEX]
        return <SID>GetHeadingLinkGFM(a:headingName)
...
command! GenTocWiki :call <SID>GenToc(s:supportMarkdownStyles[s:WIKI_STYLE_INDEX])
